"""Action autoencoder components ported from A2A_Flow_Matching."""
...
class CNNActionEncoder(nn.Module):
    def __init__(
        ...
        if conv_output_length < 1:
            raise ValueError(
                f"CNNActionEncoder needs pred_horizon >= {2**num_layers}, got {pred_horizon}"
            )
...
class MLPActionEncoder(nn.Module):
    """Simple MLP encoder for short action sequences (e.g., n_obs_steps < 8)."""
...
def make_action_encoder(
    ...
    """CNN when seq_length >= 8 (3× stride-2), otherwise MLP."""
    if seq_length >= 8:
        return CNNActionEncoder(
            pred_horizon=seq_length,
            action_dim=action_dim,
            latent_dim=latent_dim,
            hidden_dim=hidden_dim,
            num_layers=3,
        )
    return MLPActionEncoder(
