#!/usr/bin/env python3
"""最小 OpenArm Mini -> HCX 单臂 limited 遥操作基线。

本示例只保留一条控制链路：一个 OpenArm Mini 以 100 Hz 读取七轴角度，一个
HCX 机械臂以 500 Hz 调用 ``DirectServoSession.set_target()``。发送线程持续以
最新的 100 Hz 主臂目标驱动 ``LimitedInterpolator``，执行低通、限速和限加速度；
主臂没有新样本时，继续向上一个目标平稳收敛并保持发送，满足直伺服心跳要求。
...
运行：
    ...
    python -m examples.test_openarm_hcx_single_arm_direct_teleop
"""
...
# 主臂只读采样频率，单位为 Hz。
LEADER_SAMPLE_RATE_HZ = 200.0
# 每次 OpenArm 读取允许的最长等待时间，单位为秒。超时后保持上一次从臂目标。
LEADER_READ_TIMEOUT_S = 0.01

# HCX 直接伺服目标发送频率，单位为 Hz。
...
LIMITED_MAX_ACCELERATION_DEG_S2 = 120.0
# LimitedInterpolator 的固定低通权重，范围 [0, 1]；越小低通越强。
LIMITED_LOWPASS_ALPHA = 0.1
...
class DemoConfig:
    """单臂 limited 遥操作所需的最小配置，所有关节角度均为度。"""
...
class LatestTarget:
    ...
    写入者始终发布一个新数组，读取者从不原地修改该数组；因此 CPython 下的
    引用替换可作为最小的无锁最新值交接。发送线程每个 500 Hz 周期都读取最新
    值并由自身的 LimitedInterpolator 生成下一点；该槽不保存历史轨迹。
    """
...
def _run_direct_servo_sender(
    ...
    """独立 500 Hz 发送线程；每周期生成一个 limited 关节目标并下发。"""
...
def run_demo(config: DemoConfig) -> None:
    """连接一对同侧主从臂并运行 100 Hz 采样、500 Hz limited 下发。"""
    ...
    try:
        ...
        # 本示例不会自动处理示教器、报警或使能；这里仅确认现场已经可运动。
        ...
        # 先发送当前从臂姿态；首个有效主臂样本到来前，500 Hz 线程持续保持该姿态。
        ...
        sender_thread = threading.Thread(
            ...
            name=f"openarm-hcx-{config.side}-direct-servo",
            daemon=True,
        )
        sender_thread.start()

        print("=" * 72)
        print("    OpenArm Mini -> HCX 单臂最小 limited 遥操作")
        print("=" * 72)
        print(f"  侧别: {config.side}；HCX robot_id: {config.hcx_robot_id}")
        print(f"  主臂采样: {config.leader_sample_rate_hz:.0f} Hz")
        print(
            f"  HCX set_target: {config.direct_servo_rate_hz} Hz（最新目标 limited 输出）"
        )
        print(f"  轴方向: {list(config.axis_sign)}")
        print("  映射: 相对同序七轴 + 100 Hz 目标 -> 500 Hz 低通/限速/限加速度")
        print(
            "  limited: "
...
def main() -> int:
    """执行文件顶部定义的单臂硬件基线；不加载 YAML，也不解析 CLI。"""
    ...
    try:
        ...
        print(f"[ERROR] 单臂最小 limited 遥操作失败: {exc}")
        return 1
    return 0
