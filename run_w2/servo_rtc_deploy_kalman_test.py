...
def _traj_log_worker(log_q: Any, stop_evt: Any, out_dir: str) -> None:
    ...
    while True:
        ...
        np.savez_compressed(
            path / f"{infer_i:06d}.npz",
            infer_i=np.int32(infer_i),
            t=np.float64(item["t"]),
            q_now=item["q_now"],
            model=item["model"],
            rtc=item["rtc"],
            filtered=item["filtered"],
            delay=np.int32(item["delay"]),
            skip=np.int32(item["skip"]),
            leftover=np.int32(item["leftover"]),
            infer_ms=np.float32(item["infer_ms"]),
        )
...
def _infer_worker(
    ...
    try:
        ...
        while infer_i < max_steps and not stop_evt.is_set():
            ...
            model_phys = np.array(processed, dtype=np.float32, copy=True)
            if filter_enabled:
            ...
            filtered_phys = np.array(processed, dtype=np.float32, copy=True)
            ...
            with action_queue.lock:
                if action_queue.queue is None:
                    rtc_phys = np.zeros((0, int(processed.shape[1])), dtype=np.float32)
                else:
                    rtc_phys = (
                        action_queue.queue.detach()
                        .cpu()
                        .numpy()
                    ...
                    log_q.put(
                        {
                            "infer_i": infer_i,
                            "t": time.time(),
                            "q_now": np.array(q_now, dtype=np.float32, copy=True),
                            "model": model_phys,
                            "filtered": filtered_phys,
                            "rtc": rtc_phys,
                            "delay": int(new_delay),
                            "skip": int(skip),
...
def main() -> None:
    ...
    try:
        ...
        log_q = ctx.Queue(maxsize=64)
        ready_evt = ctx.Event()
        stop_evt = ctx.Event()
        traj_dir = SCRIPT_DIR / "traj_logs" / time.strftime("%y%m%d_%H%M%S")
        log_proc = ctx.Process(
            target=_traj_log_worker,
            args=(log_q, stop_evt, str(traj_dir)),
            name="traj-log",
            daemon=True,
        )
        log_proc.start()
        print(f"[INFO] 轨迹日志 {traj_dir}", flush=True)
        spec = {
            "ckpt": str(ckpt_path),
        ...
        infer_proc = ctx.Process(
            ...
            args=(spec, joint_arr, popped_val, ack_val, out_q, ready_evt, stop_evt, log_q),
            name="infer-worker",
            daemon=True,
        )
        infer_proc.start()
        ...
        if log_q is not None:
            try:
                log_q.put(None, timeout=1.0)
            except Exception:
                pass
        if log_proc is not None:
            log_proc.join(timeout=5.0)
            if log_proc.is_alive():
                log_proc.terminate()
                log_proc.join(timeout=2.0)
        if left_follower is not None:
            try:
